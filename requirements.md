...
> **「提案書に書いた内容＝実装時に守る約束」**（講師フィードバックより）
...
> 「Slack を失敗させたら本当に再送されるか」まで確認すること（講師フィードバックより）。
...
| Git 管理外 | `CLAUDE.md`・`tasks.md`・`.env.local` | 内部実装指示・進行管理・APIキーを公開リポジトリに含めない |
