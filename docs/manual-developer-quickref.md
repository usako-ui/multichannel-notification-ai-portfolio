...
作業中にパッと開く要点集約。エンドポイント / 環境変数 / トラブル判断 / デプロイ前チェックを 1 枚に凝縮しています。

**想定シーン：**
- 障害対応中に「今すぐ何を確認すべきか」を判断する
- デプロイ前に「絶対に忘れてはいけないこと」をチェックする
- 新規参画者が「まず何がどこにあるか」を掴む

📖 **詳細な手順・OAuth フロー・実行コマンド**：[`manual-developer.md`](./manual-developer.md)（詳細版）
